...
#define SYS_PLL2_PFD2 6

#define FLEXSPI_SOURCE_CLOCK 360000000
...
static void fhc_assign_pins_to_gpio(void)
{
	...
	IOMUXC_SetPinMux(IOMUXC_GPIO_SD_B2_06_GPIO10_IO15, 1);
	IOMUXC_SetPinMux(IOMUXC_GPIO_SD_B2_07_GPIO10_IO16, 1);
	IOMUXC_SetPinMux(IOMUXC_GPIO_SD_B2_08_GPIO10_IO17, 1);
	IOMUXC_SetPinMux(IOMUXC_GPIO_SD_B2_09_GPIO10_IO18, 1);
	IOMUXC_SetPinMux(IOMUXC_GPIO_SD_B2_10_GPIO10_IO19, 1);
	IOMUXC_SetPinMux(IOMUXC_GPIO_SD_B2_11_GPIO10_IO20, 1);
}
...
static void fhc_assign_pins_to_flexspi(void)
{
	...
	IOMUXC_SetPinConfig(IOMUXC_GPIO_SD_B2_06_FLEXSPI1_A_SS0_B, 0x0A); 
	IOMUXC_SetPinConfig(IOMUXC_GPIO_SD_B2_07_FLEXSPI1_A_SCLK, 0x0A); 
	IOMUXC_SetPinConfig(IOMUXC_GPIO_SD_B2_08_FLEXSPI1_A_DATA00, 0x0A); 
	IOMUXC_SetPinConfig(IOMUXC_GPIO_SD_B2_09_FLEXSPI1_A_DATA01, 0x0A); 
	IOMUXC_SetPinConfig(IOMUXC_GPIO_SD_B2_10_FLEXSPI1_A_DATA02, 0x0A); 
	IOMUXC_SetPinConfig(IOMUXC_GPIO_SD_B2_11_FLEXSPI1_A_DATA03, 0x0A);
}
...
void fhc_jreset()
{
	...
	GPIO_PinInit(GPIO10, 15, &jreset_pin_config); // CS
	GPIO_PinInit(GPIO10, 17, &jreset_pin_config); // SI/IO0
	GPIO_PinInit(GPIO10, 16, &jreset_pin_config); // SCK
	...
	GPIO_WritePinOutput(GPIO10, 16, 0); // set SCK low
	for(i = 0; i < 4; i++)
	{
		// drive CS low
		GPIO_WritePinOutput(GPIO10, 15, 0);
		// drive SI low or high: alternate its state every iteration
		GPIO_WritePinOutput(GPIO10, 17, (i&1));
		// drive CS high; SI state will be captured on the CS rising edge
		GPIO_WritePinOutput(GPIO10, 15, 1);
	}
...
void fhc_misc_reset(void)
{
	...
	GPIO_WritePinOutput(GPIO10, 17, 1);
	GPIO_WritePinOutput(GPIO10, 18, 1);
	GPIO_WritePinOutput(GPIO10, 19, 1);
	GPIO_WritePinOutput(GPIO10, 20, 1);
	// drive SCK low
	GPIO_WritePinOutput(GPIO10, 16, 0);
	...
	for(i = 0; i < 2; i++)
	{
		GPIO_WritePinOutput(GPIO10, 16, 1);
		GPIO_WritePinOutput(GPIO10, 16, 0);
	}
...
uint32_t fhc_init(void)
{
	...
	flexspi_handle = FLEXSPI1;

	flexspi_root_clock = SCLK_FREQ;
	deviceconfig.flexspiRootClk = flexspi_root_clock;

	// Set divider for source clock  to generate FlexSPI root clock
    CLOCK_SetRootClockDiv(kCLOCK_Root_Flexspi1, FLEXSPI_SOURCE_CLOCK/flexspi_root_clock);
	// Set FlexSPI clock source to SYS_PLL2_PFD2
    CLOCK_SetRootClockMux(kCLOCK_Root_Flexspi1, SYS_PLL2_PFD2);
...
static const opc_to_lutidx_t opc_to_lutidx[] =
{
