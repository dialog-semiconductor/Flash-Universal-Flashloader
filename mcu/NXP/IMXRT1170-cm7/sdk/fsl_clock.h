...
#define FSL_CLOCK_DRIVER_VERSION (MAKE_VERSION(2, 1, 10))
...
static inline void CLOCK_SetRootClockMux(clock_root_t root, uint8_t src)
{
    ...
    CCM->CLOCK_ROOT[root].CONTROL =
        (CCM->CLOCK_ROOT[root].CONTROL & ~(CCM_CLOCK_ROOT_CONTROL_MUX_MASK))
...
static inline void CLOCK_SetRootClockDiv(clock_root_t root, uint8_t div)
{
    ...
    CCM->CLOCK_ROOT[root].CONTROL =
        ...
        | CCM_CLOCK_ROOT_CONTROL_DIV(div - 1);
    __DSB();
    __ISB();
#if __CORTEX_M == 4
    (void)CCM->CLOCK_ROOT[root].CONTROL;
...
{
    return ((CCM->CLOCK_ROOT[root].CONTROL
            & CCM_CLOCK_ROOT_CONTROL_DIV_MASK) >> CCM_CLOCK_ROOT_CONTROL_DIV_SHIFT) + 1;
}
...
static inline void CLOCK_SetRootClockMFx(clock_root_t root, uint32_t mfn, uint32_t mfd)
{
    CCM->CLOCK_ROOT[root].CONTROL =
        (CCM->CLOCK_ROOT[root].CONTROL &
         ~(CCM_CLOCK_ROOT_CONTROL_MFN_MASK | CCM_CLOCK_ROOT_CONTROL_MFD_MASK))
        | (CCM_CLOCK_ROOT_CONTROL_MFN(mfn) | CCM_CLOCK_ROOT_CONTROL_MFD(mfd));
    __DSB();
    __ISB();
#if __CORTEX_M == 4
    (void)CCM->CLOCK_ROOT[root].CONTROL;
